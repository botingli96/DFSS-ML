...
target_data_tensor = torch.tensor(target_data, dtype=torch.float32).to(device)
...
input_data_train_tensor = torch.tensor(input_data_train, dtype=torch.float32).to(device)
input_data_test_tensor = torch.tensor(input_data_test, dtype=torch.float32).to(device)
target_data_train_tensor = torch.tensor(target_data_train, dtype=torch.float32).to(device)
target_data_test_tensor = torch.tensor(target_data_test, dtype=torch.float32).to(device)
...
neural_net = posterior_nn(model='nsf',  # Consider 'maf' (Masked Autoregressive Flow) or 'nsf' (Neural Spline Flow)
                          ...
                          num_transforms=10).to(device)  # Increase the number of transformations
...
density_estimator = inference.train(max_num_epochs=10, learning_rate=0.0001)  # Increase epochs for potentially better results
...
posterior.to(device)  # Move the model to GPU
...
for i in tqdm(range(len(train_target_data_subset)), desc="Processing samples", leave=True, file=sys.stdout):
    test_input = target_data_test[i].to(device)  # This is the X and I for this test sample
