...
name_format = "GBM_XGB_grid_run19"
print("Note: use results8.mat which has 10000 datasets; B only has positive Z values", flush=True)
...
param_grid = {
    ...
    'learning_rate': [0.04],        # Different learning rates to test
    'max_depth': [5],                     # Depth of trees
    'n_estimators': [3000, 3500, 4000],           # Number of boosting rounds
}
...
grid_search.fit(input_data_train, target_data_train, 
                eval_set=[(input_data_test, target_data_test)],
                verbose=True)

# Output the best parameters and best score
print("Best parameters found: ", grid_search.best_params_)
print("Best cross-validation score (negative MSE): {:.4f}".format(grid_search.best_score_))
...
mse = mean_squared_error(target_data_test, y_pred)
mae = mean_absolute_error(target_data_test, y_pred)
r2 = r2_score(target_data_test, y_pred)
