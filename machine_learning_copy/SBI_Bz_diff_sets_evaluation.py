...
name_format = "SBI_Bz_run14_new"
print("Note: use results33.mat as train data, results37.m as test data; B only has positive Z values, learning rate 0.0001, batch size 32.")
...
target_data_test_raw = np.column_stack((B_MAG_data_noisy[:, 2], EDC_MAG_data_noisy))
input_data_test_raw = I_data_noisy
...
print(f"Using {len(input_data_test_raw)} noisy samples for testing.")
...
target_data_train_np = np.hstack((B3_scaled_train, E1_scaled_train, E2_scaled_train, E3_scaled_train))
target_data_test_np = np.hstack((B3_scaled_test, E1_scaled_test, E2_scaled_test, E3_scaled_test))
print("Target data preprocessing complete.")

# --- 6. Convert Final Numpy Arrays to PyTorch Tensors ---
...
posterior_path = os.path.join("/home/botingl/machine learning copy/SBI_Bz_noise.pt")
...
train_subset_indices = np.random.choice(len(input_data_train_np), size=1000, replace=False)
...
import time 
predictions_train = []
for i in tqdm(range(len(input_data_train_np)), desc="Processing samples", leave=True, file=sys.stdout):
    train_input = input_data_train_np[i]  # This is the X and I for this test sample
...
B_true_train = scaler_B3.inverse_transform(target_data_train_np[:, 0].reshape(-1, 1))
E_true_train = np.column_stack((
    scaler_E1.inverse_transform(target_data_train_np[:, 1].reshape(-1, 1)),
    scaler_E2.inverse_transform(target_data_train_np[:, 2].reshape(-1, 1)),
    scaler_E3.inverse_transform(target_data_train_np[:, 3].reshape(-1, 1))
))
...
# Define a handler to force jump to the next sample
